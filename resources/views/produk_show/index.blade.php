    ...
    <script>
        ...
        $('#city, #courier').change(function () {
            ...
            if (city_id && courier) {
                $.post('/cost', {
                    _token: '{{ csrf_token() }}',
                    origin: 5, // Example origin city ID
                    destination: city_id,
                    courier: courier,
                    weight: weight,
                })
                .done(function (res) {
                    console.log("Cost API response:", res);

                    let services = [];

                    if (Array.isArray(res)) {
                        // response array langsung dipakai
                        services = res;
                    } else if (res && typeof res === "object") {
                        // response object tunggal dibungkus array
                        services = [res];
                    }
                    ...
                    if (services.length > 0) {
                        $('#courier_service')
                            .prop('disabled', false)
                            .empty()
                            .append('<option value="">Pilih Layanan Kurir</option>');

                        services.forEach(function (service) {
                            $('#courier_service').append(
                                '<option value="' + service.service + '" data-cost="' + service.cost + '">' +
                        ...
                        $('#courier_service')
                            .prop('disabled', true)
                            .empty()
                            .append('<option value="">Tidak ada layanan tersedia</option>');
                    }
                })
